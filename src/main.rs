    ...
    fn change_value_others_possibility(&mut self) {
        ...
        for _ in 0..3 {
            for _ in 0..3 {
                ...
                for row in 0..3 {
                    for column in 0..3 {
                        match &self.grid[row + row_jump][column + column_jump]
                            .possibilities
                            .clone()
                        {
                            Some(possibilities) => {
                ...
                for row in 0..3 {
                    for column in 0..3 {
                        if let Some(possibilities) = &self.grid[row + row_jump]
                            [column + column_jump]
                            .possibilities
                            .clone()
                        {
                            for k in possibilities {
                                if q.iter().find(|&&x| x == *k).is_none() {
                                    self.grid[row + row_jump][column + column_jump].value = *k;
                                    self.grid[row + row_jump][column + column_jump].possibilities =
                                        None;
                                    self.mutated_state = true;
                                }
                            }
        ...
        for row in 0..9 {
            for column in 0..9 {
                match &mut self.grid[row][column].possibilities {
                    Some(possibilities) => {
                        for (i, q) in zx.iter().enumerate().take(9) {
                            if i != row {
                                let q = q[column].value;
                                possibilities.retain(|x| *x != q);
                            }
                        }
                    }
